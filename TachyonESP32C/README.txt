...
This is TachyonESP built on ESP32forth v7.0.7.15 which is being modified to be more like Tachyon Forth so
...
NOTE: Use an ANSI compatible terminal to make full use of console features. Baud rate is irrelevant with this USB serial connection.
...
You may have to install python3-serial
...
Make sure you set your terminal to ANSI emulation with color enabled. In minicom on Linux I have this script:
minicom --color=on -w -b 921600 -D /dev/tty$1
This is in the Dropbox folder as tty so that you can type:
./tty ACM0
which will setup minicom correctly except to change from VT102 to ANSI with ^A T A
Since there is no UART in the serial path, the baud rate setting is ignored.
...
CONSOLE
* History buffer - use nav keys to recall - del at end of line to delete that history
* Line edit - use left/right/ins/del etc
